...
default function Header() {
 ...
 // naÄti pÅ™ihlÃ¡Å¡enÃ©ho + profil/roli
 useEffect(() => {
   let mounted = true;
   ...
   // reaguj na zmÄ›ny auth stavu (login/logout)
   ...
   <header className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b">
     <div className="max-w-6xl mx-auto px-3 h-14 flex items-center justify-between">
       ...
       <nav className="hidden md:flex items-center gap-1">
         <NavLink href="/" active={isActive("/")}>DomÅ¯</NavLink>
         ...
         {/* pÅ™ihlÃ¡Å¡enÃ½: profil + odhlÃ¡sit (dropdown) */}
         {me && (
     ...
     {menuOpen && me && (
       <div className="md:hidden border-t bg-white">
         <div className="px-3 py-2 flex flex-col">
           ...
           <Link href="/dashboard" className="px-2 py-2 rounded-md hover:bg-gray-50">
             NovÃ½ ÄlÃ¡nek
           </Link>
           {me.role === "admin" && (
             <Link href="/admin" className="px-2 py-2 rounded-md hover:bg-gray-50">
               Admin
             </Link>
           )}
