...
async function generateDraft() {
  ...
  if (postsErr) {
    return NextResponse.json({ ok: false, step: "select_posts", error: postsErr.message }, { status: 400 });
  }
  ...
  if (error) {
    return NextResponse.json({ ok: false, step: "insert_issue", error: error.message }, { status: 400 });
  }
  ...
  if (!fromCron) {
    if (!secret) {
      return NextResponse.json(
        { ok: false, reason: "missing-secret", haveEnv },
        { status: 403 }
      );
    }
    if (!haveEnv) {
      return NextResponse.json(
        { ok: false, reason: "server-missing-CRON_SECRET" },
        { status: 500 }
      );
    }
...
// POST: manuální spuštění z admin UI (Bearer token + role=admin)
export async function POST(req: Request) {
  ...
  const { data: prof } = await userClient.from("profiles").select("role").eq("id", me.user.id).single();
