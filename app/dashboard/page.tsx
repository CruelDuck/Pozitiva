...
export default function Dashboard() {
  ...
  async function createPost(e: React.FormEvent) {
    ...
    if (!form.title.trim()) { setMsg("Vyplň titulek."); return; }
    ...
    try {
      const { data: post, error } = await supabase
        ...
        .select("*")
        .single();
      if (error || !post) throw new Error(error?.message || "Chyba uložení");

      // kategorie
      if (selectedCats.length) {
        const { error: pcErr } = await supabase.from("post_categories").insert(
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <section className="bg-white border rounded-xl p-4">
        ...
        <form onSubmit={createPost} className="space-y-3">
          ...
          <input className="w-full border rounded-md p-2" placeholder="Krátký výtah (volitelné)" value={form.excerpt} onChange={(e) => setForm({ ...form, excerpt: e.target.value })} />
            ...
            <div className="space-y-2">
              {sources.map((s, i) => (
                <div key={i} className="grid grid-cols-1 md:grid-cols-5 gap-2">
                  <input className="border rounded-md p-2 md:col-span-2" placeholder="Název (volitelné)" value={s.title} onChange={(e) => setSource(i, "title", e.target.value)} />
                  <input className="border rounded-md p-2 md:col-span-3" placeholder="URL (povinné, pokud chcete uložit zdroj)" value={s.url} onChange={(e) => setSource(i, "url", e.target.value)} />
                  {sources.length > 1 && (
                    <button type="button" className="text-xs text-red-600 underline" onClick={() => removeSource(i)}>
                      Odebrat
                    </button>
                  )}
                </div>
              ))}
              <button type="button" onClick={addSourceField} className="text-sm underline">Přidat další zdroj</button>
            </div>
          </div>

          <button className="px-3 py-2 rounded-lg bg-brand-600 text-white text-sm hover:bg-brand-500 disabled:opacity-50" disabled={saving}>
            Uložit návrh
          </button>
