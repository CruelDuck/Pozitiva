...
export default async function PostDetail({ params }: { params: { id: string } }) {
  ...
  const [{ data: author }, { data: sources }] = await Promise.all([
    supabase
      .from("profiles")
      .select("display_name, username, avatar_url, bio, website_url")
  ...
  return (
    <div className="prose max-w-none">
      <h1>{post.title}</h1>
      {post.image_url ? (
        <img src={post.image_url} alt={post.title} className="rounded-lg" />
      ) : null}
      ...
      {/* Autor */}
      {author && (
        <div className="mt-8 p-4 border rounded-xl bg-white not-prose">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="font-semibold">
                {author.display_name || author.username || "Autor"}
              </div>
